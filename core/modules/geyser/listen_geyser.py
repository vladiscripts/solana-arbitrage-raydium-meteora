"""
Monitor Solana transactions for new reserve mints using Yellowstone gRPC.
"""
...
import sys
sys.path.append("./")
import modules.geyser.generated.geyser_pb2 as geyser_pb2
import modules.geyser.generated.geyser_pb2_grpc as geyser_pb2_grpc
import modules.geyser.generated.solana_storage_pb2 as solana_storage_pb2
...
class ReserveMonitor:
    ...
    ACCOUNT = '5jpgzZ3oRhnc3tTtZ3KgjYNVNhN4akhPRA62sSMqVBGk'
    INSTRUCTION_PREFIX = bytes([24, 30, 200, 40, 5, 28, 7, 119])
    ...
    def request_iterator(self) -> Iterator[geyser_pb2.SubscribeRequest]:
        ...
        request.accounts["dlmm"].account.extend([self.ACCOUNT])
        request.commitment = self.COMMITMENT_LEVEL
        print("Subscribing to reserve changes...")
        print(request)
        yield request
    ...
    def handle_update(self, update: geyser_pb2.SubscribeUpdate) -> None:
        ...
        new reserve mint instruction based on our instruction discriminator prefix.

        Args:
            update: Update message from the gRPC subscription
        """
        if not self._is_valid_reserve_transaction(update):
            return
        ...
        for instruction in message.instructions:
            if instruction.data.startswith(self.INSTRUCTION_PREFIX):
    ...
    def _is_valid_reserve_transaction(self, update: geyser_pb2.SubscribeUpdate) -> bool:
        """
        Validate if the update contains a relevant reserve transaction and our filter, which we defined 
        ...
        Returns:
            bool: True if the update contains a valid reserve transaction
        """
        return (
    ...
    async def start_monitoring(self) -> None:
        """
        Start monitoring for reserve transactions.
...
def main():
    ...
    monitor = ReserveMonitor(
        "YOUR_SOLANA_RPC_URL:10000",
        "YOUR_SOLANA_RPC_API_KEY"
    )
    try:
