...
class DLMM:
    ...
    def swap_ixs(self, in_token: Pubkey, out_token: Pubkey, in_amount: int, min_out_amount: int, lb_pair: Pubkey,  user: Pubkey,  userTokenIn: Pubkey,  userTokenOut: Pubkey, binArrays: List[Pubkey]):
        '''
        Swap tokens.
        ...
        try:
            data = json.dumps({
                ...
                "lbPair": str(lb_pair),
                "userPublicKey": str(user),
                "userTokenIn": str(userTokenIn),
                "userTokenOut": str(userTokenOut),
                "binArrays": list(map(lambda x: str(x), binArrays))
            })
            result = self.__session.post(f"{API_URL}/dlmm/swap_ixs", data=data).json()
            return result
        except requests.exceptions.HTTPError as e:
            raise HTTPError(f"Error swapping: {e}")
        except requests.exceptions.ConnectionError as e:
...
class DLMM_CLIENT:
    ...
    @staticmethod
    def get_active_bin(dlmm: DLMM) -> ActiveBin:
        '''
        Retrieve the active bin ID and its corresponding price using an instance of DLMM.

        Args:
            dlmm (DLMM): The DLMM object instance.
        
        Returns:
            ActiveBin: The active bin.
        '''
        if not isinstance(dlmm, DLMM):
            raise TypeError("dlmm must be an instance of the DLMM class")
        
        return dlmm.get_active_bin()
    
    @staticmethod
    def get_all_bins(dlmm: DLMM, min_bins: int, max_bins: int) -> GetBins:
        '''
        Retrieve all bins using an instance of DLMM.
        ...
        '''
        # if not isinstance(dlmm, DLMM):
        #     raise TypeError("dlmm must be an instance o0f the DLMM class")
        
        return dlmm.get_bins_around_active_bin(min_bins, max_bins)
    
    @staticmethod
    def get_price_to_price_per_lamports(dlmm: DLMM, price: float) -> float:
        '''
        Convert a price to price per lamports using an instance of DLMM.
        ...
        Returns:
            float: The price per lamports.
        '''
        if not isinstance(dlmm, DLMM):
            raise TypeError("dlmm must be an instance of the DLMM class")
        
        return dlmm.to_price_per_lamport(price)
    
    @staticmethod
    def get_bin_id_from_price(dlmm: DLMM, price: float, min: bool = False) -> int | None:
        '''
        Get bin ID from price using an instance of DLMM.
        ...
        return dlmm.get_bin_id_from_price(price, min)
    
    @staticmethod
    def get_swap_bin_array(dlmm: DLMM, swap_Y_to_X: bool, count: Optional[int] = 4) -> List[dict]:
        '''
        Get bin array for swap using an instance of DLMM.
        ...
        Returns:
            List[dict]: The bin array.
        '''
        if not isinstance(dlmm, DLMM):
            raise TypeError("dlmm must be an instance of the DLMM class")
        
        return dlmm.get_bin_array_for_swap(swap_Y_to_X, count)

    @staticmethod
    def swap_ixs(dlmm: DLMM, in_token: Pubkey, out_token: Pubkey, in_amount: int, min_out_amount: int, lb_pair: Pubkey, user: Pubkey, binArrays: List[Pubkey]):
        '''
        Swap tokens using an instance of DLMM.
        ...
        return dlmm.swap_ixs(in_token, out_token, in_amount, min_out_amount, lb_pair, user, binArrays)